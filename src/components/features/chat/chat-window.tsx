...
export function ChatWindow({
  ...
  const { chatMessages, isLoading, isSending, error, sendMessage, markAsRead } =
    useChat(orderId, currentUserClerkId)
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => { if (!open) onClose() }}>
      <SheetContent
        side="right"
        className="!w-full sm:!w-[420px] sm:!max-w-[420px] p-0 flex flex-col bg-slate-50 border-l border-slate-200"
      >
        {/* ----------------------------------------------------------------- */}
        {/* Header                                                             */}
        {/* ----------------------------------------------------------------- */}
        <SheetHeader className="shrink-0 bg-white border-b border-slate-100 p-5 flex-row items-center gap-3 space-y-0">
          <Avatar className="w-10 h-10 ring-2 ring-offset-2 ring-indigo-200">
            <AvatarImage src={partnerAvatarUrl} />
            <AvatarFallback className="bg-indigo-100 text-indigo-700 font-black text-sm">
              {partnerName.slice(0, 1).toUpperCase()}
            </AvatarFallback>
          </Avatar>
          <div className="flex-1 min-w-0">
            <SheetTitle className="text-sm font-black text-slate-900 uppercase tracking-tight truncate">
              {partnerName}
            </SheetTitle>
          ...
          {!isLoading && !error && chatMessages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-300">
              ...
              <div className="text-center">
                <p className="text-xs font-black uppercase tracking-widest text-slate-400">Belum ada pesan</p>
                <p className="text-[10px] text-slate-300 mt-1">Mulai koordinasi dengan mengirim pesan pertama.</p>
              </div>
            </div>
          )}
        ...
        <div className="shrink-0 p-4 bg-white border-t border-slate-100">
          <div className={cn(
            "flex items-end gap-2 bg-slate-50 border rounded-3xl px-4 py-3 transition-all duration-200",
            "focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-sm focus-within:shadow-indigo-100"
          )}>
            <Textarea
              ref={textareaRef}
              value={inputValue}
              ...
              rows={1}
              className="flex-1 resize-none border-0 bg-transparent p-0 text-sm text-slate-800 placeholder:text-slate-400 focus-visible:ring-0 focus-visible:ring-offset-0 min-h-0 max-h-32 overflow-y-auto leading-relaxed"
              style={{ height: "auto" }}
              onInput={(e) => {
            ...
            <Button
              ...
              className={cn(
                ...
                inputValue.trim()
                  ? "bg-gradient-to-br from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 shadow-md shadow-indigo-200 scale-100"

...
export function AdminDashboard({ stats }: AdminDashboardProps) {
   ...
   const { data: withdrawalsRes, isLoading: withdrawalsLoading } = useQuery({
      ...
      queryFn: async () => {
         ...
         if (!res.ok) throw new Error("Gagal mengambil data penarikan")
         return res.json() as Promise<{ success: boolean; data: WithdrawalAdmin[] }>
      }
   })

   const respondMutation = useMutation({
      mutationFn: async ({ id, status, rejectedReason }: { id: string; status: "success" | "rejected"; rejectedReason?: string }) => {
         const res = await fetch(`/api/admin/withdrawals/${id}/respond`, {
            method: "PATCH",
            headers: {
      ...
      onSuccess: () => {
         toast.success("Berhasil memproses penarikan dana!")
      ...
      <div className="container mx-auto px-4 md:px-8 py-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
         <div className="flex flex-col gap-2 mb-10">
            <h1 className="text-3xl font-medium tracking-[-0.02em] text-foreground flex items-center gap-3">
               <ShieldCheck className="w-8 h-8 text-accent" /> Admin Dashboard
            </h1>
            <p className="text-sm text-muted-foreground">Ringkasan platform, verifikasi pendaftaran, dan moderasi.</p>
         </div>

         {/* STATS SECTION */}
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <Card className="border-muted bg-card shadow-sm rounded-[24px] overflow-hidden relative">
               ...
               <CardContent className="p-6">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Fotografer Aktif</div>
                        <div className="text-3xl font-black text-foreground mt-1">{stats.totalPhotographers}</div>
                     </div>
                  </div>
               </CardContent>
            </Card>
            ...
            <Card className="border-muted bg-card shadow-sm rounded-[24px] overflow-hidden relative">
               ...
               <CardContent className="p-6">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Mitra Terdaftar</div>
                        <div className="text-3xl font-black text-foreground mt-1">{stats.totalMitras}</div>
                     </div>
                  </div>
               </CardContent>
            </Card>
            ...
            <Card className="border-muted bg-card shadow-sm rounded-[24px] overflow-hidden relative">
               ...
               <CardContent className="p-6">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Total Transaksi Selesai</div>
                        <div className="text-3xl font-black text-foreground mt-1">Rp {stats.totalRevenue.toLocaleString('id-ID')}</div>
                     </div>
                  </div>
               </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-1 gap-12">
               ...
               <section className="space-y-6">
                  <div className="flex items-center gap-3">
                     ...
                     <h2 className="text-xl font-medium text-foreground tracking-[-0.02em]">Antrean Verifikasi Fotografer</h2>
                  ...
                  {photographers.length > 0 ? (
                           ...
                           <Card key={pg.clerkId} className="border-muted bg-card shadow-sm rounded-[24px] overflow-hidden group">
                              ...
                              <CardContent className="p-6 space-y-6">
                                 ...
                                 <div className="pt-6 flex items-center gap-3 border-t border-muted">
                                    <Button
                                       variant="outline"
                                       className="flex-1 border-muted text-destructive hover:bg-destructive/5 hover:border-destructive/10 font-bold rounded-full h-10 text-xs cursor-pointer"
                                    ...
                                    <Button
                                       className="flex-1 bg-primary hover:bg-primary/95 text-primary-foreground font-bold rounded-full h-10 text-xs shadow-sm cursor-pointer"
                     ...
                     <div className="p-10 border border-dashed border-muted rounded-[24px] bg-card text-center">
                        <p className="text-muted-foreground font-bold uppercase tracking-widest text-[9px]">Tidak ada antrean fotografer</p>
                     </div>
                  )}
               </section>
               ...
               <section className="space-y-6">
                  <div className="flex items-center gap-3">
                     ...
                     <h2 className="text-xl font-medium text-foreground tracking-[-0.02em]">Antrean Verifikasi Mitra</h2>
                  ...
                  {mitra.length > 0 ? (
                     ...
                     <div className="p-10 border border-dashed border-muted rounded-[24px] bg-card text-center">
                        <p className="text-muted-foreground font-bold uppercase tracking-widest text-[9px]">Tidak ada antrean mitra</p>
                     </div>
                  )}
               </section>
               ...
               <section className="space-y-6">
                  <div className="flex items-center gap-3">
                     ...
                     <h2 className="text-xl font-medium text-foreground tracking-[-0.02em]">Permintaan Penarikan Dana Fotografer</h2>
                     <Badge variant="outline" className="rounded-full border-muted bg-card text-muted-foreground text-xs">
                        {withdrawalsRes?.data?.filter(w => w.status === "pending").length || 0} pending
                     </Badge>
                  </div>

                  {withdrawalsRes?.data && withdrawalsRes.data.length > 0 ? (
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {withdrawalsRes.data.map((w) => (
                           <Card key={w.id} className="border-muted bg-card shadow-sm rounded-[24px] overflow-hidden group">
                              <CardHeader className="bg-[#141413] text-[#FCFBFA] p-6 relative">
                                 <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:rotate-12 transition-transform pointer-events-none">
                                    <Wallet size={60} />
                                 </div>
                                 <div className="relative z-10 flex justify-between items-start gap-4">
                                    <div>
                                       <CardTitle className="text-lg font-medium tracking-[-0.01em] truncate">{w.photographerName}</CardTitle>
                                       <CardDescription className="text-[#D1CDC7] font-medium flex items-center gap-1.5 text-xs mt-1">
                                          <Mail className="w-3.5 h-3.5" /> {w.photographerEmail || `-`}
                                       </CardDescription>
                                    </div>
                                    <Badge className={
                                       w.status === "pending"
                              ...
                              <CardContent className="p-6 space-y-6">
                                 <div className="space-y-4">
                                    <div className="bg-[#059669]/5 dark:bg-[#059669]/10 p-4 rounded-[16px] border border-[#059669]/20 flex justify-between items-center">
                                       <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Jumlah Penarikan</span>
                                       <span className="text-lg font-black text-emerald-600 dark:text-emerald-400">Rp {w.jumlah.toLocaleString('id-ID')}</span>
                                    </div>

                                    <div className="space-y-2">
                                       <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Rekening Tujuan</div>
                                       <div className="bg-background p-4 rounded-[16px] border border-muted text-xs space-y-1">
                                          <div className="flex justify-between">
                                             <span className="text-muted-foreground">Bank:</span>
                                             <span className="font-bold text-foreground">{w.bankName}</span>
                                          </div>
                                          <div className="flex justify-between">
                                             <span className="text-muted-foreground">Nomor:</span>
                                             <span className="font-bold text-foreground">{w.rekeningNumber}</span>
                                          </div>
                                          <div className="flex justify-between">
                                             <span className="text-muted-foreground">Atas Nama:</span>
                                             <span className="font-bold text-foreground">{w.rekeningName}</span>
                                          </div>
                                       </div>
                                    </div>

                                    {w.status === "rejected" && w.rejectedReason && (
                                       <div className="bg-destructive/5 dark:bg-destructive/10 p-3 rounded-[16px] border border-destructive/10 text-xs text-destructive">
                                          <strong>Alasan Ditolak:</strong> {w.rejectedReason}
                                       </div>
                                    )}

                                    <div className="flex items-center gap-2 text-[9px] font-bold text-muted-foreground uppercase tracking-widest pt-2 border-t border-muted/50">
                                       <Calendar className="w-3.5 h-3.5" /> Diajukan: <span className="text-foreground">{format(new Date(w.createdAt), "d MMM yyyy, HH:mm")}</span>
                                    </div>
                                 </div>
                                    ...
                                    <div className="pt-6 flex items-center gap-3 border-t border-muted">
                                       <Button
                                          ...
                                          onClick={() => {
                                             ...
                                             respondMutation.mutate({ id: w.id, status: "rejected", rejectedReason: reason })
                                          }}
                                       ...
                                       <Button
                                          className="flex-1 bg-emerald-600 hover:bg-emerald-600/90 text-white font-bold rounded-full h-10 text-xs shadow-sm cursor-pointer"
                                          onClick={() => {
                                             if (confirm(`Setujui penarikan dana Rp ${w.jumlah.toLocaleString('id-ID')} ke rekening ${w.bankName} ${w.rekeningNumber}?`)) {
                                                respondMutation.mutate({ id: w.id, status: "success" })
                                             }
                                          }}
                                          disabled={respondMutation.isPending}
                                       >
                     ...
                     <div className="p-10 border border-dashed border-muted rounded-[24px] bg-card text-center">
                        <p className="text-muted-foreground font-bold uppercase tracking-widest text-[9px]">Tidak ada pengajuan penarikan dana</p>
                     </div>
                  )}
               </section>
            </div>

...
import { z } from "zod"
import { and, eq, inArray, or, sql, isNull, desc } from "drizzle-orm"
import { db } from "@/db"
import {
  mitraProfiles,
...
mitraRouter.post(
  ...
  async (c) => {
    try {
      ...
      // Resolve Photographer by Username
      const [pg] = await db
        .select({ 
        ...
        .where(eq(photographerProfiles.username, body.username.toLowerCase()))
        .limit(1)

      if (!pg) {
        return c.json({ success: false, error: "Fotografer dengan username tersebut tidak ditemukan" }, 404)
      }

      if (pg.verificationStatus !== "verified") {
        return c.json(
          { success: false, error: "Fotografer belum terverifikasi oleh admin" },
          400
        )
      }
